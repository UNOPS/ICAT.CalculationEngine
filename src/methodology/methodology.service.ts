...
(this.getValue(req[num], "fc") === -999) ? value = 1 : project.fc = this.getValue(req[num], "fc");
            (this.getValue(req[num], "ncv") === -999) ? value = 1 : project.ncv = this.getValue(req[num], "ncv");
            (this.getValue(req[num], "efpkm") === -999) ? value = 1 : project.efpkm = this.getValue(req[num], "efpkm");
                ...
                for (let fu in fueldata) {
                    ...
                    for (let num in req) {
                        ...
                        if (fueldata[fu] === req[num]['fuelType'] && vehicledata[vehi] === req[num]['vehical']) {
                            ...
                            (this.getValue(req[num], "dp") > -1) ? vehicle.dp = this.getValue(req[num], "dp") : value = 1;
                            (this.getValue(req[num], "n") === -999) ? value = 1 : vehicle.n = this.getValue(req[num], "n");
...
else if ( req[num]['name'].includes(fuelType[fu])){
                            let value: number;
                            (this.getValue(req[num], "efco2xy") === -999) ? value = 1 : fuel.efco2xy = this.getValue(req[num], "efco2xy");
                            (this.getValue(req[num], "ncv") === -999) ? value = 1 : fuel.ncv = this.getValue(req[num], "ncv");
                        }
                    ...
                    for (let _req of req) {
                        ...
                        if (_route === _req['route'] && _vehicle === _req['vehical'] && _fuel === _req['fuelType']) {
                            if (currentRoute === ""){
                                vehicle.type = _vehicle
                                fuel.type = _fuel;
                                for (let code of ["fuelConsumption", "ecConsumption"]) {
                                    (this.getValue(_req, code) === -999) ? value = 1 : fuel[code] = this.getValue(_req, code)
                                }
                            }
                            currentRoute = _route;
