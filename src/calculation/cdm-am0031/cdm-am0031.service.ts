...
export class CDMAM0031Service {
  ...
  public async cal(req: CdmAm0031ReqMsg) {
    ...
    for (const y in req.projectEmission) {
      const pro = this.projectEmssion(req.projectEmission[y]);
      pe += pro;
    }
  ...
  public baseline(bass: BaseLineDto): number {
    ...
    for (const baseVehicle of bass.vehicle) {
  ...
  public calBE(vehi: VehicleDto): number {
    const unit = 1000000;
    let EF_pkm = 0;
    const pow = vehi.t + vehi.y - 1;
    const irpow = Math.pow(vehi.ir, pow);
    let SD = vehi.sd;
    ...
    if (vehi.vehicleType != VehicleTypeEnum.electric_vehicle) {
      ...
      if (EF_km == 0 || EF_km == null || EF_km == undefined) {
        EF_km =
          (vehi.sfc * vehi.fuel.ncv * vehi.fuel.ef * vehi.ninx) / vehi.nzx;
      }

      if (
        vehi.fc_mass !== 0 &&
        vehi.fc_mass !== null &&
        vehi.fc_mass !== undefined
      ) {
        this.base_FC += (vehi.fc_mass * vehi.fuel.ncv * vehi.fuel.ef) / 1000;
      } else {
        this.base_FC +=
          (vehi.fc * vehi.fuel.density * vehi.fuel.ncv * vehi.fuel.ef) / 1000;
      }
  ...
  public projectEmssion(pro: ProjectDto) {
    const unit = 0.000001;
    for (const num in pro.vehicle) {
      const ef =
        pro.vehicle[num].fuel.ef +
        pro.vehicle[num].fuel.gwp * pro.vehicle[num].fuel.ef_ch4;
      if (pro.vehicle[num].vehicleType == VehicleTypeEnum.fuel_vehicle) {
        if (
          pro.vehicle[num].fc_mass > 0 &&
          pro.vehicle[num].fc_mass !== undefined
        ) {
          this.pro_FC +=
            (pro.vehicle[num].fc_mass *
              pro.vehicle[num].fuel.ncv *
              pro.vehicle[num].fuel.ef) /
            1000;
          return (
            pro.vehicle[num].fc_mass * pro.vehicle[num].fuel.ncv * unit * ef
          );
        } else if (
          pro.vehicle[num].fc > 0 &&
          pro.vehicle[num].fc !== undefined
        ) {
          this.pro_FC +=
            (pro.vehicle[num].fc *
              pro.vehicle[num].fuel.ncv *
              pro.vehicle[num].fuel.density *
              pro.vehicle[num].fuel.ef) /
            1000;
          return (
            pro.vehicle[num].fc *
        ...
        const fc = pro.vehicle[num].fc;
        if (fc > 0 && fc !== undefined && pro.vehicle[num].fc && fc !== null) {
          return (
            pro.vehicle[num].fc *
            pro.vehicle[num].fuel.ef *
            (1 + pro.vehicle[num].fuel.tdl)
          );
        } else {
          return (
            (pro.vehicle[num].sfc *
        ...
        if (projectVehi.vehicleName == baseVehi.vehicleName) {
          ...
          if (
            ...
            (projectVehi.vehicleName == 'Car' ||
              projectVehi.vehicleName == 'Taxi')
          ) {
            const le_reb =
              (projectVehi.d * ef * (projectVehi.nzx - baseVehi.nzx + nisy)) /
              unit;
            const efbe = this.be(ef, baseVehi.v, projectVehi.v);
            const le_sp =
              (projectVehi.d * projectVehi.nzx * (ef - efbe)) / unit;
