...
export class MacController {
  ...
  public MacCalculation(
    ...
    const reduction_lev_invest = this.service.pmtCalculation(
      req.DiscountRate,
      req.project.psProjectLife,
      req.project.psTotalInvestment,
    );
    const reference_lev_invest = this.service.pmtCalculation(
      req.DiscountRate,
      req.baseline.bsProjectLife,
      req.baseline.bsTotalInvestment,
    );

    const reduction_cost = this.service.totolCost(
      reduction_lev_invest,
      req.project.psAnnualOM,
      req.project.psAnnualFuel,
      req.project.psOtherAnnualCost,
    );
    const reference_cost = this.service.totolCost(
      reference_lev_invest,
      req.baseline.bsAnnualOM,
      req.baseline.bsAnnualFuel,
      req.baseline.bsOtherAnnualCost,
    );
