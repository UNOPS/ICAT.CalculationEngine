...
export class CdmAmsIiiAkController {
  constructor(public service: CdmAmsIiiAkService) {}
  ...
  public emission(@Body() req: CdmAmsIiiAkReqMsg): CdmAmsIiiAkResMsg {
    ...
    for (const arr in req.baseline) {
      ...
      baseResponse.baseLineEmission = this.service.baselineEmission(
        req.baseline[arr],
      );
      baseResponse.projectEmission = this.service.projectEmission(
        req.project[arr],
      );
      baseResponse.emissionReduction =
        ...
        Math.max(
          baseResponse.projectEmission +
            this.service.leakageEmission(req.leakage[arr]),
          0,
        );
