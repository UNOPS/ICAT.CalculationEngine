...
export class CdmAmsIiiAkService {
  ...
  public AMSIIIEmission(req: CdmAmsIiiAkReqMsg) {
    const response: CdmAmsIiiAkResMsg = new CdmAmsIiiAkResMsg();
    const responseArray = [];
    ...
    for (const arr in req.baseline) {
      ...
      baseResponse.emissionReduction =
        ...
        Math.max(
          baseResponse.projectEmission + baseResponse.leakageEmission,
          0,
        );
  ...
  public baselineEmission(baseline: BaselineDto) {
    ...
    const fuel = pp.fuel;
    this.ncvbf = pp.ncvbf;
    const ppEmission = pp.pbf - pp.pbfsite - pp.pbfother;
    const vehicleEmission = pp.fpj * pp.fff * pp.cbf - pp.pbfother;

    this.bfy = Math.min(ppEmission, vehicleEmission);

    const emission = this.bfy * this.ncvbf * fuel.efco2;
  ...
  public projectEmission(project: ProjectDto) {
    ...
    for (const fs of project.feedstock) {
      const af1 = (fs.mpmp * fs.mmp) / (fs.mpmp * fs.mmp + fs.mpbp * fs.mbp);

      const FPBF = fs.fpbf;
      const PETR = this.transportEmission(fs.vehicle);
      const PEBP = this.productionEmission(fs.fuel, fs);
      const PEMeOH = this.methonolEmission(fs.fuel);
      const AF2 = fs.af2;
      const PEBC = this.cultivationEmission(
        fs,
        fs.soil,
        fs.stratum,
        fs.fuel,
  ...
  cultivationEmission(fs, soil, stratum, fuel, feedstock) {
    ...
    for (const _soil of soil) {
      pesa += _soil.qsa * _soil.asa * _soil.efsa;
    }

    for (const _stratum of stratum) {
      losSOC +=
        1.21 *
        _stratum.asoc *
        _stratum.socref *
        (_stratum.flub * _stratum.fmgb * _stratum.finb -
          _stratum.flup * _stratum.fmgp * _stratum.finp);
    ...
    const pesoc = Math.max((44 / 12) * (1.179 / fs.t) * losSOC, 0);
    ...
    if (pebsh < 1) {
      const defaultEmission = this.cultivationDefaultEmission(feedstock);
