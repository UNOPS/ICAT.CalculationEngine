...
export class IcatTpm2020Service extends TypeOrmCrudService<PppConversionFactor> {
  ...
  public async calculate(req: IcatTpm2020request) {
      ...
      if (
        ...
        for (const num in req.baseline.vehicle) {
          ...
          if (req.baseline.fuelUsed > 0) {
            emission = this.baseLineEmission(
        ...
        for (const num in req.baseline.vehicle) {
          const emission = this.baseLineEmissoioB(
            ...
            req.baseline.vehicle[num].fuel.ef * 1000,
          );
        ...
        for (const num in req.baseline.vehicle) {
          if (
            ...
            const fuelEnergy = this.fuelEnergyWithFC(
    ...
    if (req.projectType.projectApproch > 0) {
      if (req.projectType.projectApproch === ProjectTypeEnum.projectApproch_A) {
        ...
        if (
          req.project.fuelMixPriceElasticity != undefined &&
          req.project.fuelMixPriceElasticity != 0
        ) {
        ...
        for (const num in req.baseline.vehicle) {
          ...
          if (req.project.fuel[num].priceElasticity != 0) {
            ...
            req.project.fuel[num].type != 'diesel' ||
            req.project.fuel[num].type != 'Diesel'
          ) {
            const countriFuelPrice = await this.PPPPriceService.getPPPvalue(
              req.project.special.countryCode,
            ...
            fuelElasticity = await this.elesticPriceWithdiesel(
              req.project.fuel[num].fuelPrice,
              req.project.special.priceElasticity.capitalIncome,
              countriFuelPrice,
              req.project.special.year,
        ...
        for (const num in req.baseline.vehicle) {
          ...
          if (
            ...
            fuelEmission = this.eleEmission(
              req.baseline.vehicle[num].fuel.ef,
              req.baseline.vehicle[num].fuel.fc,
            );
          }

          const energy = this.trainPKM(
            fuelEmission,
            req.baseline.vehicle[num].fuel.ef,
            req.baseline.vehicle[num].n,
          );
        ...
        req.projectType.projectApproch === ProjectTypeEnum.simplified
      ) {
        ...
        for (const n in req.project.vehicle) {
          marketShare = this.marketShare(
            req.project.vehicle[n],
            req.project.beta,
          );
          sale = req.project.vehicle[n].vahicleSale;
          projectemission += this.ghgImpact(
            marketShare,
            sale,
            emissionReduction,
            distance,
        ...
        const response1 = new icatCATResponceMsg();
        response1.baseLineEmission = parseFloat(
          Number(baseLineEmission).toFixed(2),
        );
        response1.projectEmission = parseFloat(
          Number(baseLineEmission + ghgImpact).toFixed(2),
        );
        response1.emissionReduction = parseFloat(
          Number(-1 * ghgImpact).toFixed(2),
        );
        return response1;
      } else if (
        ...
        for (const num in req.baseline.vehicle) {
          const vkt = this.changeVehicleTravel(
            req.baseline.vehicle[num].fuel.vkt,
            fuelElasticity,
            req.project.fuel[num].fuelPrice,
  ...
  public async elesticPrice(
    ...
    const usPrice = this.consumerPriceService.getConsumervalue('US', year);
    const usPrice2016 = this.consumerPriceService.getConsumervalue('US', 2016);

    const capitalIncome = capital / ppp;

    const fuelMinPrice = this.centerValue(await usPrice, await usPrice2016, 30);
    const fuelMaxPrice = this.centerValue(await usPrice, await usPrice2016, 80);

    const capitalMin = this.centerValue(
      await usPrice,
      await usPrice2016,
      12000,
    );
    const capitalMax = this.centerValue(
      await usPrice,
      await usPrice2016,
      24000,
    );
    ...
    if (fuelMinPrice > fuelPrice && capitalIncome < capitalMin) {
      return -0.15;
    } else if (fuelMinPrice > fuelPrice && capitalIncome < capitalMax) {
      return -0.22;
    } else if (fuelMaxPrice < fuelPrice && capitalIncome < capitalMin) {
      return -0.26;
    } else if (fuelMaxPrice < fuelPrice && capitalIncome > capitalMax) {
      return -0.33;
    } else if (
      fuelMinPrice > fuelPrice &&
      capitalIncome < capitalMax &&
      capitalIncome > capitalMin
    ) {
      return -0.11;
    } else if (
      fuelMaxPrice < fuelPrice &&
      capitalIncome < capitalMax &&
      capitalIncome > capitalMin
    ) {
      return -0.32;
  ...
  public async elesticPriceWithdiesel(
    fuelPrice: number,
    capital: number,
    ppp: number,
    year: number,
  ) {
    const usPrice = await this.consumerPriceService.getConsumervalue(
      'USA',
      year,
    );
    const usPrice2016 = await this.consumerPriceService.getConsumervalue(
      'USA',
      2016,
    );
    ...
    const fuelMinPrice = this.centerValue(await usPrice, await usPrice2016, 80);
    ...
    if (fuelMinPrice > fuelPrice && capitalIncome < capitalMin) {
      ...
      return -0.13;
    } else if (fuelMinPrice < fuelPrice && capitalIncome < capitalMin) {
      return -0.38;
    } else if (fuelMinPrice < fuelPrice && capitalIncome > capitalMax) {
      return -0.27;
    }
  ...
  public async crossElesticPriceWithdiesel(
    fuelPrice: number,
    capital: number,
    ppp: number,
    year: number,
    ...
    if (vehicleType === 'Bus') {
      if (fuelPrice < fuelMinPrice && capitalIncome < capitalMin) {
        return 0.09;
      } else if (fuelPrice < fuelMinPrice && capitalIncome > capitalMax) {
        return 0.14;
      } else if (fuelPrice > fuelMaxPrice && capitalIncome < capitalMin) {
        return 0.16;
      } else if (fuelPrice > fuelMaxPrice && capitalIncome > capitalMax) {
        return 0.21;
      } else if (
        fuelPrice < fuelMinPrice &&
        capitalMin < capitalIncome &&
        capitalIncome < capitalMax
      ) {
        return 0.07;
      } else if (
        fuelPrice > fuelMaxPrice &&
        capitalMin < capitalIncome &&
        capitalIncome < capitalMax
      ) {
        return 0.2;
  ...
  public fuelEnergyWithVKT(
    ...
    const unit_conversion = 1000000000;
    const energy = (vkt * sfc * density * ncv) / unit_conversion;
    return this.emission(ef, energy);
  }

  public fuelEnergyWithFC(
    fc: number,
    density: number,
    ncv: number,
    ef: number,
    ...
    const unit_conversion = 1000000000;
    const energy = (fc * density * ncv) / unit_conversion;
    return this.emission(ef, energy);
  }
  ...
  public eleEmission(ef: number, fc: number) {
    const unit_conversion = 0.0036; //MWh - Tj
  ...
  public emissionReduction(
    fuelVehicleEmission: number,
    fuelVehicleVKT: number,
    eleVehicleEmission: number,
    eleVehicleVKT: number,
  ) {
    const fuelEmission = fuelVehicleEmission / fuelVehicleVKT;
    const eleEmission = eleVehicleEmission / eleVehicleVKT;
  ...
  public baseLineEmissoioB(
    fuelUsedWeight: number,
    fuelUseLiter: number,
    density: number,
    ncv: number,
    ef: number,
  ...
  public AnticipatedCalculation(
    fuelElasticity: number,
    mixPrice: number,
    energy: number,
  ) {
    const presentage = 100;
